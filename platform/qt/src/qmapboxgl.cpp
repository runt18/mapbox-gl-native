...
/*!
    ...
    This enum sets the orientation of the north heading. It will directly affect bearing when
...
/*!
    Cycle through several debug options like showing the tile borders,
...
/*!
    ...
    \brief the map current latitude in degrees.

    Setting a latitude doesn't necessarily means it will be accepted since QMapboxGL
    might constrain it within the limits of the Mercator projection.
*/
...
/*!
    ...
    Setting a longitude doesn't necessarily means it will be accepted since QMapboxGL
    might constrain it within the limits of the Mercator projection.
*/
...
/*!
    ...
    \brief the map pitch in radians.

    Pitch toward the horizon measured in radians, with 0 resulting in a
    two-dimensional map. It will be constrained at 0.60
...
/*!
    Adds an \a className to the list of active classes. Classes are like filters, layers
    tagged with a certain class will only be active when the class is added.
...
/*!
    ...
    functions to shown some drawing on the map by referencing the \a name.

    This is different from addIcon() and it is part of the Annotation API. Annotations will
    survive style changes and Runtime Style won't.
...
/*!
    ...
    Useful for setting a new reference center of the map.
*/
